...
1. Confirm that the destination is persistent or publishable. If it is ordinary conversation, stop using this skill even when the prompt explicitly names it. 평소처럼 입력과 일반 지식만으로 직접 답한다. 이 스킬을 적용하기 위해 workspace를 조사하거나 과거 프로젝트 사실·파일 링크를 추가하지 않는다. 스킬 이름, 활성화 여부, 적용하지 않은 이유를 언급하지 않는다.
